...
public class FiltrationDeviceScreen extends AbstractContainerScreen<FiltrationDeviceMenu> {
  ...
  public FiltrationDeviceScreen(FiltrationDeviceMenu menu, Inventory inv, Component title) {
    super(menu, inv, title);
    this.imageHeight = 165;
    this.inventoryLabelY = this.imageHeight - 94;
    components.add(new TankComponent(menu.getInputTank(), 44, 23, menu.getBlockEntity().getBlockPos()));
    components.add(new TankComponent(menu.getOutputTank(), 116, 23, menu.getBlockEntity().getBlockPos()));
    components.add(new ArrowComponent(() -> menu.getProgress(), () -> 200, 77, 33, TEXTURE));
  }
