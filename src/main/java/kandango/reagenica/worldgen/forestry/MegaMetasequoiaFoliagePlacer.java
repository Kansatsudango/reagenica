...
public class MegaMetasequoiaFoliagePlacer extends FoliagePlacer{
  public static final Codec<MegaMetasequoiaFoliagePlacer> CODEC = 
        RecordCodecBuilder.create(
          instance -> foliagePlacerParts(instance)
            .and(Codec.INT.fieldOf("height").forGetter(p -> p.height))
            .apply(instance, MegaMetasequoiaFoliagePlacer::new)
        );

  private final int height;
  public MegaMetasequoiaFoliagePlacer(IntProvider radius, IntProvider offset, int height){
    super(radius, offset);
    this.height = height;
  }
  ...
  protected void createFoliage(@Nonnull LevelSimulatedReader level, @Nonnull FoliageSetter setter, @Nonnull RandomSource random,
    ...
    for(int y=0;y<foliageHeight;y++){
      ...
      this.placeLeavesRow(level, setter, random, config, layerPos, radius, 0, true);
    }
  }

  @Override
  public int foliageHeight(@Nonnull RandomSource random, int trunkHeight, @Nonnull TreeConfiguration config) {
    return Math.min(trunkHeight-4, this.height);
  }
