...
import kandango.reagenica.screen.NylonBagMenu;
...
public class NylonBag extends Item{
  ...
  public InteractionResultHolder<ItemStack> use(@Nonnull Level level, @Nonnull Player player, @Nonnull InteractionHand hand) {
      if (!level.isClientSide) {
          ...
          NetworkHooks.openScreen((ServerPlayer) player, new SimpleMenuProvider(
                  (windowId, inventory, p) -> new NylonBagMenu(windowId, inventory, slot),
                  Component.translatable("gui.reagenica.nylon_bag")
  ...
  public @Nullable ICapabilityProvider initCapabilities(ItemStack stack, @Nullable CompoundTag nbt) {
    ...
    return new NylonBagProvider(nbt);
  }

  public static class NylonBagProvider implements ICapabilityProvider, INBTSerializable<CompoundTag> {
    private final ItemStackHandler handler;
    private final LazyOptional<IItemHandler> lazyHandler;

    public NylonBagProvider(@Nullable CompoundTag nbt) {
      ...
      if (nbt != null) {
        ...
        ChemistryMod.LOGGER.debug("NylonBag NBT was null, creating new tag.");
      }
    ...
    private static boolean isAcceptableItem(ItemStack stack){
      return !stack.is(ChemiTags.Items.NYLON_BAGS);
    }
  }
}
